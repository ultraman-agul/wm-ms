      ...
      <el-tab-pane label="今日订单">
        ...
        >
          <el-table-column
            ...
            width="80"
          ></el-table-column>
          <el-table-column label="餐馆名称">
            <template #default="scope">
              {{ scope.row.restaurant.name }}
            </template>
          </el-table-column>
          <el-table-column
            prop="create_time"
...
import { getAllOrder, getTodayOrder } from '@/api/order'
...
export default defineComponent({
  setup() {
    ...
    const state = reactive({
      ...
      getTodayOrder() {
        getTodayOrder({
          offset: state.currentPage2 - 1,
          limit: state.pageSize2,
        }).then(res => {
          if (res.status === 200) {
            state.todayTableData = res.data.map(item => {
              return {
                ...item,
      ...
      handleCurrentChange2(currentPage) {
        state.currentPage2 = currentPage
        state.getTodayOrder()
      },
    })
    state.getTodayOrder()
